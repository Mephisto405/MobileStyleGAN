...
def main(args):
    ...
    for i in tqdm(range(args.n_batches)):
        var = torch.randn(args.batch_size, distiller.mapping_net.style_dim).to(
            args.device
        )
...
if __name__ == "__main__":
    ...
    # pipeline configure
    parser.add_argument(
        "--device", type=str, default="cpu", help="select device for inference"
    )
    parser.add_argument(
        "--cfg",
        type=str,
    ...
    parser.add_argument(
        "--output-path", type=str, default="./", help="path to store images"
    )
