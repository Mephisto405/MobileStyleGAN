...
class SynthesisNetwork(nn.Module):
    def __init__(
        ...
        for out_channel in channels[1:]:
            self.layers.append(
                SynthesisBlock(
                    in_channel, out_channel, style_dim, 3, blur_kernel=blur_kernel
                )
            )
    ...
    def forward(self, style, noise=None):
        ...
        if noise is None:
            _noise = torch.randn(1, 1, hidden.size(-1), hidden.size(-1)).to(
                style.device
            )
        else:
            _noise = noise[0]
        ...
        hidden = self.conv1(
            hidden, style if style.ndim == 2 else style[:, 0, :], noise=_noise
        )
