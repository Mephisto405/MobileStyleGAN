...
class FusedLeakyReLU(nn.Module):
    ...
    def forward(self, input):
        return fused_leaky_relu(
            input, self.bias, self.negative_slope, self.scale, self.trace_model
        )
...
def fused_leaky_relu(
    ...
    if input.device.type == "cpu":
        ...
        if trace_model:
            return (
                F.leaky_relu(input + bias.view(1, input.size(1)), negative_slope=0.2)
                * scale
            )
        else:
            return (
